...
(function($, undefined) {
	...
	$.extend(Stream.prototype, {
		...
		options: {
			...
			// Whether the message length is provided by the stream
			messageLength: true,
			// Message separator
			messageSeparator: ";",
		}, 
		...
		handleResponse: function(text) {
			if (this.readyState === 0) {
				...
				this.id = text.substring(0, text.indexOf(this.options.messageSeparator));
				this.message = {index: text.indexOf(this.options.messageSeparator, 
						this.id.length + this.options.messageSeparator.length) + 1, size: null, data: ""};
			...
			for(;;) {
				...
				if (this.message.size == null) {
					// Checks a separator of size part
					var sizeEnd = text.indexOf(this.options.messageSeparator, this.message.index);
					if (sizeEnd < 0) {
						return;
					}
					
					if (this.options.messageLength) {
						this.message.size = +text.substring(this.message.index, sizeEnd);
						this.message.index = sizeEnd + this.options.messageSeparator.length;
					} else {
						this.message.size = sizeEnd - this.message.index;
					}
				}
				...
				var dataEnd = text.indexOf(this.options.messageSeparator, this.message.index);
				if (dataEnd < 0) {
					return;
				}
				this.message.index = dataEnd + this.options.messageSeparator.length;

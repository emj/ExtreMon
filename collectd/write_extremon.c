...
static int we_send_buffer(we_callback_t *cb) 
{
	...
	int bytessent=sendto(cb->socket, cb->send_buffer, cb->send_buffer_fill,0,NULL,0);
	if(bytessent==-1)
    	{
...
static int we_set_max_msg_size(we_callback_t *cb, int requested_size)
{
	...
	if(setsockopt(cb->socket, SOL_SOCKET, SO_SNDBUF, (int *)&optval, optlen)==-1)
    	{
                ERROR ("setting max_msg_size on socket failed");
        	return (-1);
    	}

	if(getsockopt(cb->socket, SOL_SOCKET, SO_SNDBUF, (int *)&optval, &optlen)==-1)
    	{
	...
	if(optval!=requested_size)
    	{
                ERROR ("setting max_msg_size on socket failed: system limited to %d", optval);
        	return (1);
    	}
...
static int we_callback_init(we_callback_t *cb) 
{
	...
	u_char ttl=0;
	if(setsockopt(cb->socket, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl))==-1)
	{
                ERROR ("write_extremon plugin: setting multicast scope to host-only failed.");
		return (-1);
	}

	if(connect(cb->socket, (struct sockaddr *)&si_other, sizeof(si_other))==-1)
	{
...
static int we_flush_nolock (cdtime_t timeout, we_callback_t *cb) 
{
        ...
        DEBUG("write_extremon plugin: we_flush_nolock: timeout = %.3f; send_buffer_fill = %zu;", CDTIME_T_TO_DOUBLE (timeout), cb->send_buffer_fill);
...
static int we_write_command (const data_set_t *ds, const value_list_t *vl, we_callback_t *cb)
{
        char values[1024];
        size_t values_len;
	...
	{
                ERROR ("write_extremon plugin: DS type does not match value list type");
                return -1;
        }
	...
	if(now.tv_sec>cb->last_write.tv_sec || (now.tv_sec==cb->last_write.tv_sec && now.tv_usec > (cb->last_write.tv_usec + 100000)))
	{
        ...
        DEBUG ("write_extremon plugin: <%s> buffer %zu/%zu (%g%%) \"%s\"", cb->location, cb->send_buffer_fill, sizeof (cb->send_buffer), 100.0 * ((double) cb->send_buffer_fill) / ((double) sizeof (cb->send_buffer)), values);
...
static int we_config (oconfig_item_t *ci) 
{
	...
	for(i=0;i<ci->children_num;i++)
	{
		oconfig_item_t *child = ci->children + i;
			...
			ERROR ("write_extremon plugin: Invalid configuration option: %s.", child->key);
	}

        DEBUG ("write_extremon: Registering listener %s:%d", cb->hostname, cb->port);

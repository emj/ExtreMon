...
public class X3Drain extends Loom implements Launcher {
    private static final Logger LOGGER = Logger.getLogger(X3Drain.class
	    .getName());
	...
	this(prefix, url, Loom.DEFAULT_MAX_SHUTTLE_SIZE,
		Loom.DEFAULT_MAX_SHUTTLE_AGE);
    }

    public X3Drain(String prefix, URL url, int maxShuttleSize,
	    double maxShuttleAge) {
	...
	LOGGER.finest("setting SSLSocketFactory to "
		+ socketFactory.toString());
	...
	try {
	    ...
	    try {
		HttpsURLConnection connection = (HttpsURLConnection) this.url
			.openConnection();
		...
		connection.setRequestProperty("Content-Type",
			"application/x-extremon-shuttle");
		connection.setRequestProperty("Content-Length",
			Integer.toString(shuttleBytes.length));
		...
		int responseLength = connection.getHeaderFieldInt(
			"Content-Length", 0);
		if (responseLength > 0) {
		    ...
		    try {
			if (responseStream.read(this.responseBuffer, 0,
	    ...
	    LOGGER.log(Level.SEVERE,
		    "ExtreMon Requires UTF-8 Encoding Capability", ueex);
	    return;
	}
    }
}

...
public class Loom implements Runnable {
	...
	if (launcher == null)
	    throw new NullPointerException(
		    "A Loom Requires a Non-null Launcher");
	...
	this.outQueue.offer(new X3Record(makeLabel(label), value
		.toLowerCase().replace('=', ':').replace('\n', ' ')));
	return this;
    }

    public Loom put(String label, STATE state) {
	this.outQueue.offer(new X3Record(makeLabel(label), String
		.valueOf((int) state.getCode())));
	return this;
    }
	...
	this.outQueue.offer(new X3Record(makeLabel(label), value ? "1"
		: "0"));
	return this;
    }
    ...
    private void reset() {
	this.nextDeadline = System.currentTimeMillis()
		+ this.maxShuttleAge;
    }
	...
	for (;;) {
		...
		if (this.lazy) {
		    ...
		    record = this.outQueue.poll(this.maxShuttleAge,
		...
		if (record == this.requestStop)
		    ...
		    this.shuttle
			    .put(record.getLabel(), record.getValue());

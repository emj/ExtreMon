...
public class XKMS2Probe extends X3Out {
    private static final String XKMS2_REASONURI_PREFIX 
	...
	this.returnRevocationData = confBool("return.revocation.data",
		false);
	this.expectedFailure = confStr("expected.failure") != null ? confStr(
		"expected.failure").toLowerCase()
		: null;
	this.running = false;
	...
	while (this.running) {
		...
		trustService.validate(this.domain, this.certChain,
		...
		if (this.expectedFailure == null) {
		    ...
		    put(RESULT_COMMENT_SUFFIX, "validates (expected ["
			    + this.expectedFailure
			    + "]) trust service compromised?");
		}
		...
		put(RESULT_SUFFIX, STATE.ALERT);
		put(RESULT_COMMENT_SUFFIX, "trust domain [" + this.domain
			+ "] not found.");
		...
		if (this.expectedFailure != null) {
			...
			if (reason.toLowerCase().endsWith(
				this.expectedFailure)) {
			    expected = true;
			    continue;
			}
		...
		if (expected) {
		    put(RESULT_SUFFIX, STATE.OK);
		    put(RESULT_COMMENT_SUFFIX, "rejects with ["
			    + getPrintableReasons(ex.getReasons())
			    + "] ok");
		} else {
		    put(RESULT_SUFFIX, STATE.ALERT);
		    put(RESULT_COMMENT_SUFFIX, "rejects with ["
	...
	for (int i = 0; i < reasons.size(); i++) {
		...
		builder.append(reason.substring(XKMS2_REASONURI_PREFIX
			.length()));
		if (i < (reasons.size() - 1))
		    builder.append(',');
	    }

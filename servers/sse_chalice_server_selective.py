...
#
#   SSE Server by Koen De Causmaecker
# 	koendc@gmail.com
#
...
class SSESelectiveChaliceRequestHandler(BaseHTTPRequestHandler):
		...
		for line in str(data,'UTF-8').splitlines():
			if len(line)>0:
				...
				if label and value:
					...
					if select:
						self.shuttle.add('data: %s=%s' % (label,value))
...
class SSESelectiveChaliceServer(ThreadingMixIn, ChaliceServer,HTTPServer):
	def __init__(self,listen,prefix):
		HTTPServer.__init__(self,listen, SSESelectiveChaliceRequestHandler)
		ChaliceServer.__init__(self, prefix)

class SSESelectiveChaliceDispatcher(Thread):
	def __init__(self,mcast_addr,listen,prefix):
		Thread.__init__(self)
		self.daemon=True
		self.server=SSESelectiveChaliceServer(listen,prefix)
...
if __name__=='__main__':
	...
	SSESelectiveChaliceDispatcher(mcast_addr=('224.0.0.1',1249),listen=(listen_addr,listen_port),prefix=prefix)

...
class CauldronSubscriber:
    """ subscribe to the Cauldron at a certain endpoint, handler is called with shuttles boiling at that port, as lists of label-value tuples
        ...
        and a handler instance, which should implement a handle_shuttle(self,dict) method,
        then call receive_forever (which blocks): your handle_shuttle method will be called
        with a list of label-values for each shuttle that boils at the endpoint you gave.
    """
    def __init__(self,context,cauldron,handler):
        self.sub_socket=context.socket(zmq.SUB)
        self.sub_socket.connect(cauldron)
